...
JointInfluence meshletDecodeJointInfluence(uint16_t data) {
  return JointInfluence(bitfieldExtract(uint(data), 12, 4),
    float(bitfieldExtract(uint(data), 0, 12)) / 4095.0f);
}
